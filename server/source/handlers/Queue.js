...
class Queue extends EventEmitter {
   ...
   async play(playable, metadata) {
      try {
         ...
         const data = JSON.parse(fs.readFileSync('tracks.json', 'utf8')) || [];
         const cached = data.find((item) => item.id === track.id);

         // if (cached) return this.play(this.next());

         let channel, message;
         let youtube, query, result;
         if (cached) {
            channel = await this.client.channels.fetch('1343313574898040862');
            message = await channel.messages.fetch(cached.message);
            track = cached.track;
         } else {
            youtube = this.client.search.youtube;

            query = `${track.name} - ${track.artist} Auto-generated by YouTube`;
         ...
         if (cached) {
            ...
            if (!response.ok) throw new Error('Error fetching audio stream');
         ...
         const ffmpegStream = fluent({ source: stream })
         ...
         const resource = Voice.createAudioResource(ffmpegStream.stream(), {
            inlineVolume: true,
            inputType: 'webm/opus',
            metadata: track,
         });
